class Mage < Hero
  def initialize(hero_exists, hero:nil, sex:"mage", rasse:"human", classe:"mage", name:"dummy", user_id:"")
    if hero_exists
      super
  ...
  def take_damage(damage:, blocked_positions:[], position:, info:{})
    ...
    if blocked_positions.include?(position);
      ...
      damage -= taken
      if(damage <= @properties['damage_reduce'])
        damage = 0
        @properties['damage_reduce'] -= damage
      else
        damage -= @properties['damage_reduce']
        @properties['damage_reduce'] = 0
      end
    end
  ...
  def generate_mana(info:{})
    ...
    @properties['damage_reduce'] += (@defencive_improvements['damage_reduce_per_turn'] * @properties['intelligence']).floor
